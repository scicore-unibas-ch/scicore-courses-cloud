...
2026-09-20 teardown). Run `playbooks/cleanup.yml` first (it deletes the image
too - it holds the cluster's munge key and is useless once the cluster is
gone; `-e '{"slurm_cleanup_images": []}'` keeps it), then destroy, then
re-run destroy to confirm 0 resources.
